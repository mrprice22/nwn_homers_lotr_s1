...
string LORD_SZGRAVE_01 = "RIP <%name%>, you will not be forgotten!";
...
int GyardCreateRandomTreasure(object oTarget)
{
    ...
    switch (n)
    {
        ...
        case 7:  sTag = "deadbaby"; break; // dead baby
...
void main()
{
    ...
    {
       effect eDmg = EffectDamage(d6(20));
       object oBlood;
       int n = Random(7);
       effect eFingerLost = EffectAbilityDecrease (ABILITY_DEXTERITY, 1);
       object oRing = GetItemInSlot (INVENTORY_SLOT_RIGHTRING);
       ...
       switch (n)
       {
           ...
           case 4:
                ...
                DelayCommand(0.6f,FloatingTextStringOnCreature("*Squish*", oUser));
                DelayCommand(0.6f,CreatePLCWrapper("plc_bloodstain",GetLocation(oUser),10.0f));
                DelayCommand(1.0f, AssignCommand(oUser,ActionSpeakString("Oh my God, I just chopped my finger off!")));
                ApplyEffectToObject( DURATION_TYPE_PERMANENT, eFingerLost, oUser);
                CreateItemOnObject("severedfinger", oUser);
                SetLocalInt(oUser,"severedfinger",1);
                ActionUnequipItem (oRing);
                break;
           case 5:
               AssignCommand(oUser,ActionPlayAnimation(ANIMATION_FIREFORGET_PAUSE_SCRATCH_HEAD));
