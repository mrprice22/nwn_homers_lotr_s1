...
// * Removes all properties from oItem
void CIRemoveItemProps(object oItem);

// *  Return a copy of oItem appearing like oNewItem and destroy oItem
object CICopyItemAppearance(object oItem, object oNewItem);
...
object CICopyItemAppearance(object oItem, object oNewItem)
{
    ...
    if (oNew != OBJECT_INVALID)
    {
        if (CIGetCurrentModMode(oPC) == X2_CI_MODMODE_ARMOR)
        {
            for (nPart = 0; nPart<ITEM_APPR_ARMOR_NUM_MODELS; nPart++)
            {
                nNewApp = GetItemAppearance(oNewItem,ITEM_APPR_TYPE_ARMOR_MODEL, nPart);
                if (nNewApp!=GetItemAppearance(oItem,ITEM_APPR_TYPE_ARMOR_MODEL, nPart))
                {
                    oTemp=CopyItemAndModify(oNew, ITEM_APPR_TYPE_ARMOR_MODEL, nPart, nNewApp, TRUE);
        ...
        else if (CIGetCurrentModMode(oPC) == X2_CI_MODMODE_WEAPON)
        {
            for (nPart = 0; nPart<=2; nPart++)
            {
                nNewApp = GetItemAppearance(oNewItem,ITEM_APPR_TYPE_WEAPON_MODEL, nPart);
                if (nNewApp!=GetItemAppearance(oItem,ITEM_APPR_TYPE_WEAPON_MODEL, nPart))
                {
                    oTemp=CopyItemAndModify(oNew, ITEM_APPR_TYPE_WEAPON_MODEL, nPart, nNewApp, TRUE);
                    if (oTemp != OBJECT_INVALID)
                    {
                        DestroyObject(oNew);
                        oNew=oTemp;
