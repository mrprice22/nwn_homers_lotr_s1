...
void main()
{
  ...
  int iLevelAffected = 21;   // The min. PC Level at which the Rest-restrictions will be applied (1-20; default=2)
  int iRestDelay = 0;       // The ammount of hours a player must wait between Rests (Default = 8 hours)
  ...
  if (GetHitDice (oPC) >= iLevelAffected) // Check if the rest-restrictions will be applied to the player
  {
    ...
    if (GetLastRestEventType() == REST_EVENTTYPE_REST_STARTED)
    {
      ...
      else // Resting IS possible
      {
        ...
        { // Resting IS NOT allowed
          AssignCommand (oPC, ClearAllActions()); // Prevent Resting
          SendMessageToPC (oPC, "You can't rest here, there are Hostiles nearby.");
        }
      }
    }
